report 50106 "Trial Balance 2"
{
    ...
    // (INC) KPMG (DPM) 24/09/21 Quitamos el filtro porque lo está blanqueando al no funcionar el GETFILTER
    //                            Añadimos el filtro a la columna de saldo
    // KPMG (DPM) 04/10/21 Sacamos la cuenta 5505007 al final del balance por solicitud del cliente
    DefaultLayout = RDLC;
    RDLCLayout = './src/Reports/TrialBalance2.rdlc';
    Caption = 'Trial Balance', Comment = 'ESP="Balance de sumas y saldos"';
    ApplicationArea = All;
    ...
    dataset
    {
        dataitem("G/L Account"; "G/L Account")
        {
            ...
            column(G_L_Account_Source_Code_Filter; SourceCode + ': ' + vFiltroSourceCode)
            {
            }
            ...
            dataitem(Integer; Integer)
            {
                ...
                column(NetChange5505007; NetChange5505007)
                {
                }
                ...
                column(Account5505007; Account5505007)
                {
                }
                column(Name5505007; Name5505007)
                {
                }
                column(Credit5505007; Credit5505007)
                {
                }
                column(Debit5505007; Debit5505007)
                {
                }
                column(Credit55050072; Credit55050072)
                {
                }
                column(Debit55050072; Debit55050072)
                {
                }
                column(NetChange55050072; NetChange55050072)
                {
                }
            ...
            begin
                ...
                IF vFiltroSourceCode <> '' THEN
                    "G/L Account".COPYFILTER("G/L Account"."Source Code Filter", GLAcc2."Source Code Filter");
                //(INC) S2G (JDT) 16-03-20:
                ...
                IF "G/L Account"."No." = '5505007' THEN BEGIN
                    Account5505007 := "G/L Account"."No.";
                    Name5505007 := "G/L Account".Name;
                    Debit5505007 := "G/L Account"."Debit Amount";
                    Credit5505007 := "G/L Account"."Credit Amount";
                    "G/L Account".CALCFIELDS("Net Change");
                    NetChange5505007 := "G/L Account"."Net Change";
                    Debit55050072 := GLAcc2."Debit Amount" + OpenDebitAmtEnd + CloseDebitAmt;
                    Credit55050072 := GLAcc2."Credit Amount" + OpenCreditAmtEnd + CloseCreditAmt;
                    NetChange55050072 := BalanceType;
                END;
                GLAcc3.RESET();
                GLAcc3.COPYFILTERS("G/L Account");
                GLAcc3.SETCURRENTKEY("No.");
                GLAcc3.SETFILTER("Debit Amount", '<>%1', 0);
                IF GLAcc3.FINDLAST() THEN
                    EVALUATE(DebitLast, GLAcc3."No.");
                GLAcc3.RESET();
                GLAcc3.COPYFILTERS("G/L Account");
                GLAcc3.SETCURRENTKEY("No.");
                GLAcc3.SETFILTER("Credit Amount", '<>%1', 0);
                CLEAR(isLast);
                IF GLAcc3.FINDLAST() THEN
                    EVALUATE(CreditLast, GLAcc3."No.");
                IF CreditLast < DebitLast THEN BEGIN
                    IF FORMAT(DebitLast) = "G/L Account"."No." THEN
                        isLast := TRUE;
                END ELSE BEGIN
                    IF FORMAT(CreditLast) = "G/L Account"."No." THEN
                        isLast := TRUE;
                END;
                IF GLAcc3."No." = "G/L Account"."No." THEN
                    isLast := TRUE;
                //++KPMG (DPM)
            ...
            begin
                ...
                IF GUIALLOWED THEN
                    //++(INC) KPMG (DPM) 24/09/21
                    IF PrintRegEntries = TRUE
                    THEN
                        "G/L Account".SETFILTER("Source Code Filter", '')
                    ELSE BEGIN
                        "G/L Account".SETFILTER("Source Code Filter", '<>ASTOREGUL');
                        //--(INC) KPMG (DPM) 24/09/21
        ...
        layout
        {
            area(content)
            {
                group(Options)
                {
                    ...
                    field(ShowRegEntries; PrintRegEntries)
                    {
                        Caption = 'Include Regularization Entries', Comment = 'ESP="Incluir movimientos de regularización"';
                        ApplicationArea = All;
                    }
                }
            }
    ...
    var
        ...
        PrintRegEntries: Boolean;
        SourceCode: Label 'Cod. Origen', Comment = 'ESP="Cod. Origen"';
        BalanceatPeriodCaptionLbl: Label 'Balance at Period', Comment = 'ESP="Saldo en periodo"';
        Account5505007: Code[10];
        Credit5505007: Decimal;
        Debit5505007: Decimal;
        Name5505007: Text;
        NetChange5505007: Decimal;
        l: Integer;
        K: Integer;
    ...
    begin
        ...
        IF AccPeriod3.FIND('+') THEN BEGIN
            ...
            //(INC) S2G (JDT) 16-03-20:
            IF vFiltroSourceCode <> '' THEN
                GlAccount3.SETFILTER("Source Code Filter", vFiltroSourceCode);
            //(INC) S2G (JDT) 16-03-20:
    ...
    begin
        ...
        //(INC) S2G (JDT) 16-03-20:
        IF vFiltroSourceCode <> '' THEN
            GlAccount4.SETFILTER("Source Code Filter", vFiltroSourceCode);
        //(INC) S2G (JDT) 16-03-20:
    ...
    begin
        ...
        //(INC) S2G (JDT) 16-03-20:
        IF vFiltroSourceCode <> '' THEN
            GLAcc.SETFILTER("Source Code Filter", vFiltroSourceCode);
        //(INC) S2G (JDT) 16-03-20:
